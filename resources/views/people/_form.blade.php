...
{{-- 基本情報 --}}
<h2 class="text-sm font-bold text-forest-800 border-b border-forest-100 pb-2 mb-4">基本情報</h2>

<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
        <label class="block text-sm font-medium text-forest-800">姓 <span class="text-red-500">*</span></label>
    ...
    <div>
        <label class="block text-sm font-medium text-forest-800">名 <span class="text-red-500">*</span></label>
    ...
    <div>
        <label class="block text-sm font-medium text-forest-800">旧姓・養子縁組前の姓</label>
    ...
    <div>
        <label class="block text-sm font-medium text-forest-800">生物学的性別 <span class="text-red-500">*</span></label>
        <select name="biological_sex" class="mt-1 block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">
    ...
    <div>
        <label class="block text-sm font-medium text-forest-800">生年月日</label>
    ...
    <div>
        <label class="block text-sm font-medium text-forest-800">出生地</label>
...
<h2 class="text-sm font-bold text-forest-800 border-b border-forest-100 pb-2 mb-4 mt-8">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div class="flex items-center gap-2">
        <input type="checkbox" name="is_deceased" value="1" id="is_deceased"
            class="rounded border-forest-300 text-forest-700 focus:ring-forest-600"
        ...
        <label for="is_deceased" class="text-sm font-medium text-forest-800">死亡している</label>
    </div>

    <div id="death_date_field" class="{{ old('is_deceased', $person->is_deceased ?? false) ? '' : 'hidden' }}">
        <label class="block text-sm font-medium text-forest-800">死亡日</label>
...
{{-- 血縁関係 --}}
<h2 class="text-sm font-bold text-forest-800 border-b border-forest-100 pb-2 mb-4 mt-8">血縁関係</h2>

<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
        <label class="block text-sm font-medium text-forest-800">父</label>
        <div class="flex items-center gap-2 mt-1">
            <select name="biological_father_id" id="biological_father_id"
                class="block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">
            ...
            <button type="button" onclick="openQuickAddModal('biological_father_id')"
                class="text-xs font-bold text-forest-700 whitespace-nowrap hover:underline">＋新規</button>
        </div>
        <div class="flex items-center gap-2 mt-2">
            ...
            <label for="is_father_adopted" class="text-xs text-forest-700/70">父とは養子縁組による関係</label>
        </div>
        @error('biological_father_id') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
    </div>

    <div>
        <label class="block text-sm font-medium text-forest-800">母</label>
        <div class="flex items-center gap-2 mt-1">
            <select name="biological_mother_id" id="biological_mother_id"
                class="block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">
                <option value="">選択なし</option>
                @foreach ($people as $candidate)
        ...
        <div class="flex items-center gap-2 mt-2">
            ...
            <label for="is_mother_adopted" class="text-xs text-forest-700/70">母とは養子縁組による関係</label>
        </div>
...
{{-- 備考 --}}
<h2 class="text-sm font-bold text-forest-800 border-b border-forest-100 pb-2 mb-4 mt-8">備考</h2>

<div>
    <textarea name="notes" rows="3"
        class="mt-1 block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">{{ old('notes', $person->notes ?? '') }}</textarea>
...
{{-- 親人物クイック登録モーダル --}}
<div id="quick-add-modal" class="hidden fixed inset-0 bg-forest-900/40 flex items-center justify-center z-50 px-4">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md">
        <h3 class="text-lg font-black text-forest-900 mb-4">新しい人物を登録</h3>
        <p class="text-xs text-forest-700/60 mb-4">氏名と性別のみの簡易登録です。詳細は後から編集画面で入力できます。</p>

        <div class="space-y-3">
            <div>
                <label class="block text-sm font-medium text-forest-800">姓 <span class="text-red-500">*</span></label>
                <input type="text" id="quick_last_name"
                    class="mt-1 block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-forest-800">名 <span class="text-red-500">*</span></label>
                <input type="text" id="quick_first_name"
                    class="mt-1 block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-forest-800">生物学的性別 <span class="text-red-500">*</span></label>
                <select id="quick_biological_sex"
                    class="mt-1 block w-full rounded-xl border-forest-200 focus:border-forest-600 focus:ring-forest-600 shadow-sm">
